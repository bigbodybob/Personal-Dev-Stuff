...
public class GameSelectionArrow : MonoBehaviour {
	...
	void Update () {
		if (GameControl.control.isPCOpen && GameControl.control.isClearBugsClicked) {
			...
			if (Input.GetKeyDown (KeyCode.DownArrow) && GameControl.control.currentGameSelectionIndex!=GameControl.control.allGames.Count-1) {
			...
			if (Input.GetKeyDown (KeyCode.UpArrow)&& GameControl.control.currentGameSelectionIndex!=0) {
